...
export class Migration1772873912613 implements MigrationInterface {
    name = 'Migration1772873912613'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE "attendance" ("id" SERIAL NOT NULL, "attendance_date" date NOT NULL, "check_in" TIMESTAMP WITH TIME ZONE, "check_out" TIMESTAMP WITH TIME ZONE, "created_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "updated_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "user_id" integer, CONSTRAINT "UQ_2f972d00331e3924686c70d2f83" UNIQUE ("user_id", "attendance_date"), CONSTRAINT "PK_ee0ffe42c1f1a01e72b725c0cb2" PRIMARY KEY ("id")); COMMENT ON COLUMN "attendance"."attendance_date" IS 'tanggal presensi'; COMMENT ON COLUMN "attendance"."check_in" IS 'waktu kehadiran'; COMMENT ON COLUMN "attendance"."check_out" IS 'waktu pulang'`);
        await queryRunner.query(`CREATE TABLE "notifications" ("id" SERIAL NOT NULL, "message" text NOT NULL, "is_read" boolean NOT NULL DEFAULT false, "created_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "user_id" integer, CONSTRAINT "PK_6a72c3c0f683f6462415e653c3a" PRIMARY KEY ("id"))`);
